        ...
        try {
            ...
            $addExoGroupMembership = Invoke-Command -Session $remoteSession -ScriptBlock {
                try {
                    ...
                    if ($auditErrorMessage -like "*Microsoft.Exchange.Management.Tasks.MemberAlreadyExistsException*") {
                        $auditLogs.Add([PSCustomObject]@{
                                Action  = "GrantPermission"
                                Message = "User $($aRef.UserPrincipalName) ($($aRef.Guid)) is already a member of the group $($pRef.Name) ($($pRef.id)). Skipped grant of permission for group $($pRef.Name) ($($pRef.id)) to user $($aRef.UserPrincipalName) ($($aRef.Guid))"
                                IsError = $false
                            }
                        )
                    }
