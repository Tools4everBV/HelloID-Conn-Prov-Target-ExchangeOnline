#####################################################
# HelloID-Conn-Prov-Target-ExchangeOnline-Create-Update-ProxyAddresses
#
# Version: 2.0.0
#####################################################
...
# Change mapping here
# Update proxy addresses with "@enyoi.nl" alias
$extraProxyAddress = $p.Accounts.MicrosoftAzureAD.userPrincipalName.Split("@")[0] + "@enyoi.nl"
$account = [PSCustomObject]@{
    # Extra proxy addresses
    EmailAddresses = @{ add = "$extraProxyAddress" }
}
...
try {
    ...
    # Update Mailbox Proxy Addresses
    try {
        $mailboxSplatParams = @{
        ...
        Write-Verbose "Updating proxy addresses for mailbox [$($aRef.userPrincipalName) ($($aRef.Guid))]: $($mailboxSplatParams | ConvertTo-Json)"

        if ($dryRun -eq $false) {
            # See Microsoft Docs for supported params https://learn.microsoft.com/en-us/powershell/module/exchange/set-mailbox?view=exchange-ps
            $updateMailbox = Set-Mailbox @mailboxSplatParams -ErrorAction Stop

            $auditLogs.Add([PSCustomObject]@{
                    Action  = "CreateAccount"
                    Message = "Successfully updated proxy addresses for mailbox [$($aRef.userPrincipalName) ($($aRef.Guid))]: $($mailboxSplatParams | ConvertTo-Json)"
                    IsError = $false
                })
        }
        else {
            Write-Warning "DryRun: would update proxy addresses for mailbox [$($aRef.userPrincipalName) ($($aRef.Guid))]: $($mailboxSplatParams | ConvertTo-Json)"
        }
    }
    catch {
        $ex = $PSItem
        ...
        $auditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Error updating proxy addresses for mailbox [$($aRef.userPrincipalName) ($($aRef.Guid))]: $($mailboxSplatParams | ConvertTo-Json). Error Message: $($errorMessage.AuditErrorMessage)"
                IsError = $True
            })
    }
}
    ...
    $result = [PSCustomObject]@{
        ...
        ExportData       = [PSCustomObject]@{
            ...
            UserPrincipalName = $mailbox.UserPrincipalName
            Guid              = $mailbox.Guid
            ProxyAddresses    = $updateMailbox.ProxyAddresses
        }
    }
